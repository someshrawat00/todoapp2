...
const getLocalData = () => {
  const lists = localStorage.getItem("personaltodo");
  if (lists) {
...
const Personal = ({ title, e }) => {
  ...
  useEffect(() => {
    localStorage.setItem("personaltodo", JSON.stringify(items))
  }, [items])
        ...
        items.map((curElem) => {
          return (
            <motion.div className={css.todocontainer} key={curElem.id}
              initial={{ y: 70 }}
              animate={{ y: 0, animationDuration: .3 }}
              exit={{ y: 0 }}
            >
